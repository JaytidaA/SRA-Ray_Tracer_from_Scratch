...
struct world {
    ...
    int cylDefined;
    cylinder cylEl[10];
    int cubeDefined; // Number of cubes defined in the world
    cube cubeEl[10]; // Array of cubes in the world
};
...
bool hitWorld(inout hit_record hr, world w, ray r, interval i) {
    ...
    for (int j = 0; j < w.cubeDefined; j++) {
        if (hitCube(temp, w.cubeEl[j], r, i)) {
            hit_anything = true;
            i.maxEl = hr.lambda;
            hr = temp;
        }
...
void main() {
    ...
    W0.sphDefined = 2;
    W0.cylDefined = 1;
    W0.cubDefined = 1; // Set to 1 to define cubes
    ...
    //W0.cubEl[0] = createCube(dvec3(0.0, -1.0, -1.0), dvec3(0.5, 0.5, 0.5), M2); // Add cube definition
